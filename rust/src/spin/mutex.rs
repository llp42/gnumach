...
use crate::spin::{RelaxStrategy, Spin};
use core::{
    fmt,
    ops::{Deref, DerefMut},
};
...
///
/// The implementation is a regular spin mutex.
///
/// # Example
///
/// ```
...
unsafe impl<T: ?Sized, R> Sync for MutexGuard<'_, T, R> where
    for<'a> &'a mut T: Sync
{
}
unsafe impl<T: ?Sized, R> Send for MutexGuard<'_, T, R> where
    for<'a> &'a mut T: Send
{
}
...
impl<T: ?Sized, R> Mutex<T, R> {
    ...
    pub unsafe fn force_unlock(&self) {
        // SAFETY: The caller guarantees the lock is held by the current
        // thread.
