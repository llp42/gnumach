...
mod status {
    ...
    impl Status {
        ...
        unsafe fn new_unchecked(inner: u8) -> Self {
            // SAFETY: The caller guarantees that `inner` is a valid
            // discriminant for `Status`.
...
impl<T, R: RelaxStrategy> Once<T, R> {
    ...
    /// }
    /// ```
    pub fn try_call_once<F: FnOnce() -> Result<T, E>, E>(
        &self,
        f: F,
    ) -> Result<&T, E> {
    ...
    #[cold]
    fn try_call_once_slow<F: FnOnce() -> Result<T, E>, E>(
        &self,
        f: F,
    ) -> Result<&T, E> {
        loop {
    ...
    pub fn poll(&self) -> Option<&T> {
        loop {
            ...
            match self.status.load(Ordering::Acquire) {
                ...
                Status::Panicked => {
                    panic!("Once previously poisoned by a panicked")
                }
            }
        }
    }
...
impl<T, R> Once<T, R> {
    ...
    unsafe fn force_into_inner(self) -> T {
        ...
        unsafe { this.data.get_mut().assume_init_read() }
    }
    ...
    pub unsafe fn get_unchecked(&self) -> &T {
        ...
        // SAFETY: The caller guarantees the `Once` has been initialized.
        unsafe { self.force_get() }
    }
