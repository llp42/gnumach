...
unsafe impl<T: ?Sized, R> Sync for FairMutexGuard<'_, T, R> where
    for<'a> &'a mut T: Sync
{
}
unsafe impl<T: ?Sized, R> Send for FairMutexGuard<'_, T, R> where
    for<'a> &'a mut T: Send
{
}
...
impl<T: ?Sized, R> FairMutex<T, R> {
    ...
    #[inline(always)]
    pub fn try_lock_starver(
        &self,
    ) -> Result<FairMutexGuard<'_, T, R>, LockRejectReason> {
        match self
            .lock
    ...
    pub fn starve(&self) -> Starvation<'_, T, R> {
        ...
        if self.lock.fetch_add(STARVED, Ordering::Relaxed)
            > (isize::MAX - 1) as usize
        {
...
impl<T: ?Sized + fmt::Debug, R> fmt::Debug for FairMutex<T, R> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        struct LockWrapper<'a, T: ?Sized + fmt::Debug, R>(
            Option<FairMutexGuard<'a, T, R>>,
        );
...
impl<'a, T: ?Sized + fmt::Display, R> fmt::Display
    for FairMutexGuard<'a, T, R>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
