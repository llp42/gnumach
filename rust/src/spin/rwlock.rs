...
// SAFETY: Write guards can be thought of as mutable reference to the inner data. In fact, this
...
unsafe impl<T: ?Sized, R> Sync for RwLockWriteGuard<'_, T, R> where
    for<'a> &'a mut T: Sync
{
}
unsafe impl<T: ?Sized, R> Send for RwLockWriteGuard<'_, T, R> where
    for<'a> &'a mut T: Send
{
}

// SAFETY: Read guards can be thought of as immutable reference to the inner data. In fact, this
// would be their ideal representation if it were not for the need for the critical section to end
// *after* the reference is no longer live.
unsafe impl<T: ?Sized, R> Sync for RwLockReadGuard<'_, T, R> where
    for<'a> &'a T: Sync
{
}
unsafe impl<T: ?Sized, R> Send for RwLockReadGuard<'_, T, R> where
    for<'a> &'a T: Send
{
}
...
// SAFETY: Read guards can be thought of as immutable reference to the inner data. In fact, this
// would be their ideal representation if it were not for the need for the critical section to end
// *after* the reference is no longer live. Note that the ability to upgrade the lock does not
// result in extra requirements: those are already assured by `RwLockWriteGuard` on upgrade.
unsafe impl<T: ?Sized> Sync for RwLockUpgradableGuard<'_, T> where
    for<'a> &'a T: Sync
{
}
unsafe impl<T: ?Sized> Send for RwLockUpgradableGuard<'_, T> where
    for<'a> &'a T: Send
{
}
...
impl<T: ?Sized, R> RwLock<T, R> {
    ...
    pub fn try_upgradeable_read(
        &self,
    ) -> Option<RwLockUpgradableGuard<'_, T, R>> {
        if self.lock.fetch_or(UPGRADED, Ordering::Acquire)
            & (WRITER | UPGRADED)
            == 0
        {
...
impl<'rwlock, T: ?Sized + fmt::Debug, R> fmt::Debug
    for RwLockReadGuard<'rwlock, T, R>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl<'rwlock, T: ?Sized + fmt::Display, R> fmt::Display
    for RwLockReadGuard<'rwlock, T, R>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl<'rwlock, T: ?Sized, R: RelaxStrategy>
    RwLockUpgradableGuard<'rwlock, T, R>
{
...
impl<'rwlock, T: ?Sized, R> RwLockUpgradableGuard<'rwlock, T, R> {
    ...
    fn try_upgrade_internal(
        self,
        strong: bool,
    ) -> Result<RwLockWriteGuard<'rwlock, T, R>, Self> {
        if compare_exchange(
            &self.inner.lock,
            UPGRADED,
            WRITER,
...
impl<'rwlock, T: ?Sized + fmt::Debug, R> fmt::Debug
    for RwLockUpgradableGuard<'rwlock, T, R>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        fmt::Debug::fmt(&**self, f)
    }
...
impl<'rwlock, T: ?Sized, R> RwLockWriteGuard<'rwlock, T, R> {
    ...
    /// ```
    #[inline]
    pub fn downgrade_to_upgradeable(
        self,
    ) -> RwLockUpgradableGuard<'rwlock, T, R> {
        debug_assert_eq!(
...
impl<'rwlock, T: ?Sized + fmt::Debug, R> fmt::Debug
    for RwLockWriteGuard<'rwlock, T, R>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        fmt::Debug::fmt(&**self, f)
    }
...
impl<'rwlock, T: ?Sized, R> Deref for RwLockReadGuard<'rwlock, T, R> {
    type Target = T;
...
impl<'rwlock, T: ?Sized, R> Drop for RwLockReadGuard<'rwlock, T, R> {
    fn drop(&mut self) {
        debug_assert!(
            self.inner.lock.load(Ordering::Relaxed) & !(WRITER | UPGRADED) > 0
        );
...
impl<'rwlock, T: ?Sized, R> Drop for RwLockWriteGuard<'rwlock, T, R> {
    fn drop(&mut self) {
        debug_assert_eq!(
            self.inner.lock.load(Ordering::Relaxed) & WRITER,
            WRITER
        );
...
mod tests {
    ...
    fn no_premature_upgrade_drop_on_failure() {
        type RwLock<T> =
            lock_api_crate::RwLock<crate::spin::rwlock::RwLock<()>, T>;
        ...
        std::thread::scope(|s| {
            ...
            let upgradable =
                match lock_api_crate::RwLockUpgradableReadGuard::try_upgrade(
                    upgradable,
                ) {
            ...
            s.spawn(|| {
                if lock.try_write().is_some() {
                    panic!(
                        "try_write should not succeed because lock is held"
                    );
                }
            })
            .join()
