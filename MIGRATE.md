...
- ~~`kern/queue.c`~~ **ported** — now `rust/src/kern/queue.rs`: a
  ...
  `queue_empty()`) and the generic `queue_enter()`,
  `queue_enter_first()`, `queue_remove()` operations, all of which are
  Rust functions now; only `queue_iterate()` remains a macro.
